...
SELECT 
    ...
    TO_VARCHAR(
        SNOWFLAKE.CORTEX.PARSE_DOCUMENT(
            '@TRUCK_WARRANTY_WORKSHOP.ANALYTICS.DOCUMENTS',  --Change if needed
            RELATIVE_PATH,
            {'mode': 'LAYOUT'}
        ):content
    ) AS EXTRACTED_TEXT
FROM DIRECTORY(@TRUCK_WARRANTY_WORKSHOP.ANALYTICS.DOCUMENTS)  --Change if needed
...
WAREHOUSE = COMPUTE_WH  --Change if needed
TARGET_LAG = '1 hour'
AS (
    SELECT
        CHUNK_TEXT,
...
SELECT PARSE_JSON(
    SNOWFLAKE.CORTEX.SEARCH_PREVIEW(
        'TRUCK_WARRANTY_WORKSHOP.ANALYTICS.DOCUMENTATION_SEARCH',  --Change if needed
        '{
